...
test('packDouble2x32', () => {
    const v = new glm.uvec2(2147483647, 1);
    const p = glm.packDouble2x32(v);
    const u = glm.unpackDouble2x32(p);

    assert.strictEqual(u.x, v.x);
    assert.strictEqual(u.y, v.y);
});

test('packHalf2x16', () => {
    const v = new glm.vec2(0.1, 0.5);
    const p = glm.packHalf2x16(v);
    const u = glm.unpackHalf2x16(p);

    assert.ok(Math.abs(u.x - 0.1) < 1e-3);
    assert.ok(Math.abs(u.y - 0.5) < 1e-3);
});
